...
var bt = class {
	...
	removeWindowInteractionListeners() {
		typeof window > "u" || (window.removeEventListener("pointermove", this.handleGlobalPointerMove), window.removeEventListener("touchmove", this.handleGlobalTouchMove), window.removeEventListener("pointerup", this.handleGlobalPointerUp));
	}
	setupInteraction() {
		let e = this.model;
		e && (e.eventMode = "static", e.automator.autoHitTest = !0, this.removeWindowInteractionListeners(), window.addEventListener("pointerup", this.handleGlobalPointerUp), this.state.isAlwaysFocus ? (window.addEventListener("pointermove", this.handleGlobalPointerMove), window.addEventListener("touchmove", this.handleGlobalTouchMove, { passive: !1 })) : e.internalModel?.focusController?.focus(0, 0, !0), e.on("pointerup", this.handleGlobalPointerUp), e.on("hit", (e) => {
	...
	async setupGestureManager() {
		if (!(!this.model || typeof window > "u") && !this.gestureManager) try {
			...
			}), this.gestureManager = { destroy: () => t(r).unset() };
		} catch {}
	}
	...
	cleanup() {
		typeof window < "u" && window.removeEventListener("resize", this.handleResize), this.removeWindowInteractionListeners(), this.resizeObserver?.disconnect(), this.resizeObserver = null, this.gestureManager &&= (this.gestureManager.destroy(), null), this.stopAudio(), this.cleanupModel();
		try {
	...
	*/
	...
	cleanupModel() {
		...
		if (this.removeOverrideHook(), this.paramOverrides.clear(), this.state.overriddenParams = [], this.frozenEffects = void 0, this.state.isFrozen = !1, this.state.motionsPaused = !1, this.originalIdleGroup = null, this.model) {
			try {
