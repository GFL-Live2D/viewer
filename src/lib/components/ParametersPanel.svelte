...
<div class="flex h-full flex-col overflow-hidden">
    ...
    {#if $controller?.state.loading === ModelLoadingState.READY}
        <!-- Button overlays the scroll area so appearing does not resize it or shift scroll position -->
        <div class="relative flex min-h-0 flex-1 flex-col">
            {#if ($controller?.state.overriddenParams.length ?? 0) > 0}
                <div class="absolute top-4 right-4 left-4 z-10">
                    <button
                        ...
                        class="border-border bg-background-tertiary text-foreground-secondary hover:bg-background-secondary hover:text-foreground hover:border-accent h-9 w-full rounded border px-3 text-xs font-medium shadow-md transition"
                    >
            ...
            <div class="custom-scrollbar flex flex-1 flex-col gap-3 overflow-y-auto p-4">
                <p class="text-foreground-tertiary shrink-0 text-xs">
