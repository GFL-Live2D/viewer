<script lang="ts">
    ...
    import { Move, Plus, Minus, RotateCcw, ImageUp } from '@lucide/svelte';
...
<button
    ...
    class="border-border bg-background-secondary/95 text-foreground-tertiary hover:border-accent hover:bg-accent/10 hover:text-foreground hidden h-10 w-10 items-center justify-center rounded-lg border shadow-lg transition-all duration-300 2xl:flex {hideUI
    ...
    <div class="flex flex-col gap-3 transition-opacity duration-300 {hideUI ? 'pointer-events-none opacity-0' : ''}">
        <button
            ...
            class="border-border flex h-10 w-10 items-center justify-center rounded-lg border shadow-lg transition {isBgMoveMode
                ...
                : 'bg-background-secondary text-foreground-tertiary hover:border-accent hover:bg-accent/10 hover:text-foreground'}"
        ...
        <button
            ...
            class="border-border bg-background-secondary text-foreground-tertiary hover:border-accent hover:bg-accent/10 hover:text-foreground flex h-10 w-10 items-center justify-center rounded-lg border shadow-lg transition"
        ...
        >
            <button
                onclick={() => (bgZoom = Math.min(10, bgZoom + 1))}
            ...
            />
            <button
                onclick={() => (bgZoom = Math.max(-10, bgZoom - 1))}
